...
ondgeojson <- merge(mgeojson, df_ond, by.x = "adm1_name", by.y = "adm1_name")
```
...
selected_severity <- reactive({
  ...
  output$lesotho_map <- renderLeaflet({
    ...
    leaflet() %>% setView(lng = 28.2336, lat = -29.6091, zoom = 7.5) %>%
      addTiles() %>%
      ...
      addPolygons(data = ondgeojson, color = "#444444", weight = 1, smoothFactor = 0.5,
