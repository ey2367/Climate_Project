...
df_main <- as.data.frame(rbind(
    ...
    c("Seasonal Impact", "Food Insecurity", '2023-11-01', '2023-12-31', 'Impact 1', "4"),
    c("Seasonal Impact", "Disease Outbreak", '2023-12-01', '2023-12-31', 'Impact 2k', "5"),
    c("Seasonal Impact", "Water Scarcity", '2023-12-01', '2023-12-31', 'Impact 3', "6"),
    c("Seasonal Impact", "Loss of Livestock", '2023-10-01', '2023-12-31', 'Impact 4', "7"),
...
sidebar <- dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Trigger Mechanism", icon = icon("magnifying-glass-chart"), tabName = "trigger"),
      menuItem("Forecasting Performance", icon = icon("droplet"), tabName = "rain"),
...
#         #   ))
    ...
    tabItem(tabName = "actions",
      ...
      fluidRow(column(width = 12,
        box(title = tags$strong("Action Descriptions", style = "font-size: 19px;"), width = 6, height = 700,
        ...
        tableOutput("action_info")),# select output
         box(title = tags$strong("Owners", style = "font-size: 19px;"), width = 6, height = 700,
        selectInput("Owner_Selection", # create a dropdown menu for user to select input
              label = "Action Type", 
              choices = c(#"Select an Owner" = "", # for multiple select + start number from 2
                          "DMA" = 1, 
                          "WFP" = 2,
                          "SD" = 3,
                          "DRWS" = 4,
...
tabItem(tabName = "data",
  ...
  tabItem(tabName = "risk",
    ...
    tabItem(tabName = "trigger",
      fluidRow(column(width = 12, # add section for supporting materials
        box(title = tags$strong("Drought Trigger Mechanism", style = "font-size: 21px;"), width = NULL, height = 500, uiOutput(""))
      ))
    )
))
...
# FOR MULTIPLE OWNER SELECTION
        #     selected_owners <- c()
        # 
        # if (2 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "DMA")
        # }
        # if (3 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "WFP")
        # }
        # if (4 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "SD")
        # }
        # if (5 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "DRWS")
        # }
        # if (6 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "WASCO")
        # }
        # if (7 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "DA")
        # }
        # if (8 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "RWS")
        # }
        # if (9 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "Min. of Health")
        # }
        # if (10 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "MAFSN")
        # }
        # if (11 %in% owner_select) {
        #   selected_owners <- c(selected_owners, "Dept. of Range")
        # }
        # 
        # if (length(selected_owners) > 0) {
        #   df_aa_rev <- df_aa_rev[df_aa_rev$Owner %in% selected_owners, ]
        # } else {
        #   # If no owner is selected, keep the original dataframe
        #   df_aa_rev <- df_aa_rev
        # }
...
# render organization/owner table based on reactive input
output$owner_info <- renderTable({
    selected_data2 <- filtered_data2() %>%
      arrange(desc(row_number()))
    selected_data2[, c("Owner", "Task", "Description")] # Only show Task, Owner, & Description
  })
