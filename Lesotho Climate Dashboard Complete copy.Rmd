...
body <- dashboardBody(use_theme(my_theme),
  ...
  p(class = "text-muted",
       ...
       fluidRow(column(width = 12,
        box(title = "", width = NULL, height = 650, plotlyOutput("fig")),
...
#         #   ))
    ...
    tabItem(tabName = "actions",
      ...
      fluidRow(column(width = 12,
        box(title = tags$strong("Action Descriptions", style = "font-size: 19px;"), width = NULL, height = 700,
...
tabItem(tabName = "rain",
        fluidRow(column(width = 12,
        box(title = tags$strong("Forecasted vs. Measured Rainfall", style = "font-size: 23px;"), width = NULL, height = 500, plotlyOutput("rain_plot"))
      ))
      ),
tabItem(tabName = "data",
  ...
  tabItem(tabName = "risk",
      fluidRow(column(width = 6,
        box(title = tags$strong("At-Risk District Populations", style = "font-size: 23px;"), width = NULL, height = 500, plotOutput("population_chart_risk")),
      ),
      column(width = 6, #output width
      box(title = tags$strong("At-Risk Districts", style = "font-size: 23px;"), width = NULL, leafletOutput("risk_map", height = 500),
            actionButton("reset_riskmap_button", "Reset Map")),
      )),
      fluidRow(column(width = 12,
        box(title = tags$strong("Council & Geographic Information", style = "font-size: 21px;"), width = NULL, height = 700,
...
ui <- dashboardPage(
  dashboardHeader(title = tags$div(HTML('<b style="font-size: 16px;">Lesotho Dashboard</b>')), 
                  titleWidth = 500,
...
# Build Lesotho map
  ...
  #filtered_data <- selected_severity()
    leaflet() %>% setView(lng = 28.2336, lat = -29.6091, zoom = 7.5) %>% # set center coordinates and zoom for initial view
      ...
      title = "Probability of Threshold Excedence (%)",
      opacity = 0.77,
...
#     df %>%
#       filter(start >= input$range[1] & end <= input$range[2])
#   }) 
...
#      if (!is.null(owner_select) && length(owner_select) > 0) {
#   owner_select <- trimws(owner_select)
#   df_aa_rev <- df_aa_rev[trimws(df_aa_rev$owner) %in% owner_select, ]
# }
...
output$district_table <- renderTable({
    selected_data1 <- filtered_data1()# %>%
      #arrange(desc(row_number()))
    selected_data1[, c("Council", "Geographic.Zone")]
  })
...
# At-risk districts population graph
 ...
 output$risk_map <- renderLeaflet({ 
  #filtered_data <- selected_severity()
    leaflet() %>% setView(lng = 28.2336, lat = -29.6091, zoom = 7.5) %>% # set center coordinates and zoom for initial view
      ...
      opacity = 1.0, fillOpacity = 0.77,
      fillColor = ~"#26828e", # Mafeteng, Mohale’s Hoek, Thaba-Tseka, and Quthing most at-risk
      highlightOptions = highlightOptions(color = "white", weight = 2,
      bringToFront = TRUE))
  })
...
output$rain_plot <- renderPlotly({
    plot_ly(data = rain_data, type = 'scatter', mode = 'lines', x = ~Year, y = ~RainHistory, name = 'Rain History', color = '#fb9b06') %>%
      add_trace(type = 'bar', x = ~Month, y = ~RainForecast, name = 'Rain Forecast', yaxis = 'y2', marker = list(color = '#21918c')) %>%
    ...
    config(displayModeBar = FALSE) %>%
    layout(margin = list(l = 50, r = 10, b = 50, t = 50),
    xaxis = list(fixedrange = TRUE),
    yaxis = list(fixedrange = TRUE),
    yaxis2 = list(fixedrange = TRUE))
  })
